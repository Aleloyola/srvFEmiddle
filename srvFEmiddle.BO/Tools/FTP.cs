...
set
{
    ...
    //for binary download
    if (value)
        ExecuteNoASCII("TYPE I");
    else
        //otherwise stay in Ascii mode
        ExecuteNoASCII("TYPE A");
...
#region Class Methods
    public FTP(string hostIP, string userName, string password, string path, ILog log)
        {
            _ftpServer = hostIP;
...
//oLog.LogInfo("Opening connection to " + _ftpServer);
...
try
{
    ...
    remoteAddress = GetIPAddress(_ftpServer);
  //  oLog.LogInfo("Se obtuvo IP");
    //set the endpoint value
    addrEndPoint = new IPEndPoint(remoteAddress, _ftpPort);
    //oLog.LogInfo("Se define endpoint");
    //connect to the ftp server
    ftpSocket.Connect(addrEndPoint);
    //oLog.LogInfo("Se conecta socket");
}
catch (Exception ex)
{
...
//oLog.LogInfo("read the host response");
//read the host response
readResponse();
//check for a status code of 220
//oLog.LogInfo("check for a status code of 220 vs:" +_statusCode.ToString());
if (_statusCode != 220)
{
    //oLog.LogError("check for a status code of 220");
    //failed so close the connection
    CloseConnection();
    //throw an exception
    //throw new FtpException(result.Substring(4));
}
//oLog.LogInfo("execute the USER ftp command (sends the username) 1");
//execute the USER ftp command (sends the username)
Execute("USER " + _ftpUsername);
//oLog.LogInfo("check the returned status code after USER execute 1");


//oLog.LogInfo("execute the USER ftp command (sends the username) 2");
//execute the USER ftp command (sends the username)
Execute("USER " + _ftpUsername);
//oLog.LogInfo("check the returned status code after USER execute 2");

//check the returned status code
if (!(_statusCode == 331 || _statusCode == 230))
{
    //oLog.LogError("status distinto a 331 y 230");
    //not what we were looking for so
    //logout and throw an exception
    LogOut();
    //throw new FtpException(result.Substring(4));
}
//oLog.LogInfo("if the status code isnt 230");
//if the status code isnt 230
if (_statusCode != 230)
{
    //oLog.LogInfo("execute the PASS ftp command (sends the password)");
    ...
    {
        //oLog.LogError("password status error");
        //not what we were looking for so
        //logout and throw an exception
        LogOut();
        //throw new FtpException(result.Substring(4));
...
oLog.LogInfo("Connected to " + _ftpServer);
...
{
    oLog.LogInfo("Init of downloadingFile function");
    //make sure the user is logged in
    if (!_isLoggedIn)
    {
        oLog.LogInfo("is not logged");
        FtpLogin();
    }

//IsBinary = true;
//display a downloading file message
oLog.LogInfo("Downloading file " + ftpFile + " from " + _ftpServer + "/" + _ftpPath);
...
{
    oLog.LogError("error al abrir el socket para transferencia: " + ex.Message);
}
...
oLog.LogInfo("Downloading " + ftpFile);
...
if (_statusCode != 150 && _statusCode != 125)
{
    ...
    oLog.LogError("code != 150 && 125, result is:"+_statusCode.ToString());
    //throw new FtpException(result.Substring(4));
}
//set a timeout value
DateTime timeout = DateTime.Now.AddSeconds(_timeOut);
...
public void ChangeWorkingDirectory(string dirName)
{
    ...
    if (dirName == null || dirName.Equals(".") || dirName.Length == 0)
    {
        //no directory was provided so throw an exception 
        //and break out of the method
        oLog.LogError("A directory name wasn't provided. Please provide one and try your request again.");
        //throw new FtpException("A directory name wasn't provided. Please provide one and try your request again.");
    }
    ...
    {
        //FtpLogin();
        oLog.LogError("You need to log in before you can perform this operation");
        //throw new FtpException("You need to log in before you can perform this operation");
    }
    ...
    if (_statusCode != 250)
    {
        oLog.LogError("check for a status code of 250");
        //operation failed, throw an exception
        //throw new FtpException("check for a status code of 250");
    }
    ...
    if (_statusCode != 257)
    {
        oLog.LogError("check for a status code of 257");
        //operation failed, throw an exception
        //throw new FtpException(result.Substring(4));
    }
    ...
    oLog.LogInfo("Current directory is " + _ftpPath);
}
#endregion
...
//oLog.LogInfo("split the host response: "+statusMessage);
...
//oLog.LogInfo("check for a space in the host response, if it exists return: "+statusMessage);
...
//oLog.LogInfo("return message: " + statusMessage);
//return the message
...
private void Execute(String msg)
{
    ...
    if (_doVerbose) oLog.LogInfo(msg);
...
private void ExecuteNoASCII(String msg)
{
    //check to see if verbose debugging is enabled
    //if so write the command to the window
    if (_doVerbose) oLog.LogInfo(msg);
...
private int[] getmsgSegments(string InternalResult)
{
    int idx1 = InternalResult.IndexOf('(');
    int idx2 = InternalResult.IndexOf(')');
    //now we need to get everything in the parenthesis
    string ipData = InternalResult.Substring((idx1 + 1), (idx2 - idx1) - 1);
    ...
    for (int i = 0; i < msgLength && partCount <= 6; i++)
    {
        ...
        else if (chr != ',')
        {
            ...
            throw new FtpException("Malformed PASV result: " + InternalResult);
        }
        else
        {
            ...
            if (chr == ',' || i + 1 == msgLength)
            {
                ...
                {
                    throw new FtpException("Malformed PASV result (not supported?): " + InternalResult, ex);
                }
            }
        }
        if (i == msgLength - 1)
...
//send the PASV command (Passive command)
    ...
    if (_statusCode != 227)
    {
        throw new FtpException("No se encontró el codigo 227: "+result);
    }
...
//enclosed in "(" & ")"
    oLog.LogInfo("Result: " + result);
    int[] msgSegments = getmsgSegments(result);
...
//string port = msgSegments[4].ToString() + msgSegments[5].ToString();
Socket tranferSocket = null;
IPEndPoint ipEndPoint = null;

try
{
    oLog.LogInfo("create our new socket for transfering data, with ip:"+ipAddress+ " Port:"+port.ToString());
	tranferSocket = new Socket(AddressFamily.InterNetwork,SocketType.Stream,ProtocolType.Tcp);

    ipEndPoint = new IPEndPoint(GetIPAddress(_ftpServer), port);
	tranferSocket.Connect(ipEndPoint);
    oLog.LogInfo("Se conecta con endpoint");
}
catch(Exception ex)
{
	// doubtfull....
